...
from app_core.tests.tx_data_for_tests import test_block_data, test_tx_data, test_strange_block_data
...
class SyncBlocksAndTxsTestCase(TestCase):
    ...
    @patch.object(Eth, 'getBlock')
    @set_mongo_test_db
    def test_sync_block_with_tx_by_number_strange_data(self, mock_get_block, mock_get_transaction):
        self.test_block_data = test_strange_block_data
        mock_get_block.return_value = copy.deepcopy(self.test_block_data)
        mock_get_transaction.return_value = copy.deepcopy(self.test_tx_data)
        ...
        self.assertEqual(db.eth_blocks.count(), 1)
        self.assertEqual(db.eth_transactions.count(), 0)
